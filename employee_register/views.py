...
def employee_list(request):
    ...
    return render(
        request,
        "employee_register/employee_list.html",
        {
...
def employee_pdf(request):
    ...
    html = template.render(
        {"employee_list": employees}, request=request  # ✅ SAME NAME
    )
...
def employee_excel(request):
    ...
    for emp in employees:
        ws.append([emp.fullname, emp.mobile, str(emp.position)])  # ForeignKey safe
...
def employee_chart(request):
    # Group data
    data = Employee.objects.values("position__title").annotate(total=Count("id"))
    ...
    return render(request, "employee_register/employee_chart.html", {"chart": graphic})
...
def dashboard(request):
    ...
    recent_employees = Employee.objects.select_related("position").order_by("-id")[:5]

    chart_data = Employee.objects.values("position__title").annotate(total=Count("id"))

    return render(
        request,
        "employee_register/dashboard.html",
        {
